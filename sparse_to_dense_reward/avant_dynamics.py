...
class AvantDynamics:
    ...
    def __init__(self, dt: float, device: str, eval: bool):
        ...
        if eval:
            self.gp_dict = {}
            for name in ["omega_f", "v_f", "dot_dot_beta"]:
    ...
    def _mpc_dynamics_fun(self, x_values: torch.Tensor, u_values: torch.Tensor) -> torch.Tensor:
        ...
        gp_dot_dot_beta = self.gp_dict["dot_dot_beta"](gp_dot_dot_beta_inputs).mean
        dot_dot_beta = nominal_dot_dot_beta + 0*gp_dot_dot_beta
