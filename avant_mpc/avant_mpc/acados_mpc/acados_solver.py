...
class AcadosSolver:
    def __init__(self, N, sample_time, sample_time2=None, cost_model_path=None, linearize_cost_model=False):
        ...
        if cost_model_path is not None:
            ...
            if linearize_cost_model:
                ...
                self.l4c_model = l4c.realtime.RealTimeL4CasADi(model, device="cpu", approximation_order=1)
    ...
    def initialize(self, initial_state, kappa_values):
        ...
        if self.l4c_model is not None and self.linearize_cost_model:
            ...
            kappa_values = np.tile(kappa_values, (self.N+1, 1))
            neural_states = np.c_[self.x0[:, :2], np.sin(self.x0[:, 2]), np.cos(self.x0[:, 2]), kappa_values[:, :2], np.sin(kappa_values[:, 2]), np.cos(kappa_values[:, 2]), self.x0[:, 3:], np.r_[self.u0, np.zeros([1, 2])]]
            neural_cost_params = self.l4c_model.get_params(neural_states)

        # Fill in x0, u0 and p for the solver:
        for i in range(self.N + 1):
            if self.l4c_model is not None and self.linearize_cost_model:
                self.acados_solver.set(i, "p", np.r_[kappa_values, self.time_steps[i], neural_cost_params[i]])
            else:
                self.acados_solver.set(i, "p", np.r_[kappa_values, self.time_steps[i]])
